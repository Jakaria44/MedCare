...
function DoctorForm({ editing = false, fieldsProp = defaultFields }) {
  ...
  const submit = async () => {
    try {
      ...
      console.log(values);
      let res;
      if (!editing) res = await api.post("/doctor/apply", values);
      else res = await api.put("/protect/doctor/update/" + values.id, values);
